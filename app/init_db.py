...
def init_db():
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute("SELECT id_utilisateur, nom_utilisateur FROM utilisateur")
            users = cursor.fetchall()

            cursor.execute("SELECT id_organisation FROM organisation")
            organisations = cursor.fetchall()

            for (user_id, nom_utilisateur) in users:
                if nom_utilisateur in ("CommercialFNAC", "CommercialGare"):
                    continue
            ...
            cursor.execute("""
                ...
                WHERE id_utilisateur IN (
                    ...
                    FROM utilisateur
                    WHERE nom_utilisateur IN (?, ?)
                )
            """, ("CommercialFNAC", "CommercialGare"))
